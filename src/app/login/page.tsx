...
export default function Login() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (response?.error) {
        ...
        else setMessage("Please Enter correct credentials");
      } else {
        router.push("/");
      }
    },
  ...
  return (
    <div className="min-h-[calc(100vh-100px)] flex justify-center items-center px-4 py-6 sm:px-6 md:px-8">
      {(open)&&(<AlertDialog open={open} setOpen={setOpen} error={error} message={message}/>)}
      <div className="w-full max-w-[90%] sm:max-w-md md:max-w-lg lg:max-w-[500px] flex justify-center items-center flex-col z-10 gap-4 sm:gap-5 md:gap-[20px]">
        ...
        >
          ...
          <Button
            ...
            type="submit"
            variant="contained"
            className="w-full !bg-purple-800 !mb-[10px] text-sm sm:text-base !text-white"
            sx={{
