...
export default function Header() {
    ...
    try {
      if (search) {
        ...
        if (response.data.data.length > 0) {
          ...
          const isDifferent = JSON.stringify(newData) !== JSON.stringify(searchBooks);
  ...
  <div onMouseDown={(e) => e.preventDefault()} className="absolute w-[240px] right-[260px] top-[65px] text-black bg-gray-200 min-h-[50px] max-h-[200px] overflow-y-scroll shadow-sm shadow-gray-400  ">
    <div className="w-full  py-[10px] ">
      {searchBooks.length > 0 ? (
        <ul className="divide-y divide-gray-100">
          {searchBooks.map((data, index) => (
            <li className="px-[20px] hover:bg-gray-300 cursor-pointer" onClick={()=>{handleBook(data.id)}} key={index}>{data.title}</li>
          ))}
        </ul>
      ) : (
          ...
          {session && status === "authenticated" ? (
            <div className="bg-gray-300 h-[40px] rounded-2xl flex justify-center items-center gap-[10px] px-4 text-black hover:bg-purple-200">
              ...
              <input
                ...
                placeholder="Search...."
                value={search}
                onFocus={() => {if(!isSearchFocused){setIsSearchFocused(true)}}}
